...
class AuthService:
    ...
    async def authenticate_user(self, payload: LoginRequest) -> TokenResponse:
        async with UnitOfWork(self.database_session) as unit_of_work:
            ...
            if not user_entity or not self.password_service.verify_password(
    ...
    async def refresh_tokens(self, refresh_token_string: str) -> TokenResponse:
        decoded_payload = self.token_service.decode_token(
        ...
        async with UnitOfWork(self.database_session) as unit_of_work:
            persisted_token = await unit_of_work.user_repository.get_refresh_token(
