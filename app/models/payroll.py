...
class PayRun(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    pay_month: Mapped[str] = mapped_column(String(7), unique=True, nullable=False) # 'YYYY-MM'
    status: Mapped[PayRunStatus] = mapped_column(SQLEnum(PayRunStatus), default=PayRunStatus.DRAFT, nullable=False)
...
class Payslip(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    pay_run_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("pay_runs.id", ondelete="CASCADE"), nullable=False)
